                        ...
                        <form id="pengguna-add-form" role="form" novalidate enctype="multipart/form-data" class="form page-form form-horizontal needs-validation" action="<?php print_link("pengguna/add?csrf_token=$csrf_token") ?>" method="post">
                            <div>
                                ...
                                <div class="form-group ">
                                    <div class="row">
                                        <div class="col-sm-4">
                                            <label class="control-label" for="parent_id">Menginduk Kepada <span class="text-danger">*</span></label>
                                        </div>
                                        <div class="col-sm-8">
                                            <div class="">
                                                <!-- <input id="ctrl-parent_id" value="<?php echo $this->set_field_value('parent_id', ""); ?>" type="text" placeholder="Enter Parent Id" required="" name="parent_id" class="form-control " /> -->
                                                <select required="" id="ctrl-role" name="parent_id" placeholder="Select a value ..." class="custom-select">
                                                    <option value="">Select a value ...</option>
                                                    <?php
                                                    $role_options = $comp_model -> parentUser();
                                                    if (!empty($role_options)) {
                                                        foreach ($role_options as $option) {
                                                            $value = $option['id'];
                                                            $label = $option['nama'];
                                                            $selected = $this->set_field_selected('parent_id', $value, "");
                                                    ?>
